...
export class WalletService {
  ...
  private async ensureCorrectNetwork(): Promise<void> {
    ...
    if (currentChainId !== environment.chainIdHex) {
      console.log('[DEBUG] Red actual:', currentChainId, '- Cambiando a', environment.chainIdHex);
      try {
        ...
        if (err.code === 4902) {
          console.log('[DEBUG] Red no encontrada, agregandola...');
          await this.ethereum.request({
  ...
  disconnect(): void {
    ...
    this._error.set(null);
    console.log('[DEBUG] Wallet desconectada');
  }
}
